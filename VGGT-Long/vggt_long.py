...
from base_models.base_model import VGGTAdapter,Pi3Adapter,MapAnythingAdapter,VGGTOmegaAdapter
...
def save_chunk_result(path, data):
    """
    Save one chunk prediction with pickle protocol 4.

    Some numpy versions hard-code pickle protocol 3 inside np.save() for
    object arrays. A VGGT-Omega chunk can exceed 4 GiB because it stores dense
    world points, depth, confidence, and images together. Protocol 3 cannot
    serialize such large byte objects, while protocol 4 can.
    """
...
def load_chunk_result(path):
    """
    Load a chunk prediction saved by save_chunk_result().

    Falls back to np.load(...).item() so older chunks saved by np.save() remain
    readable.
    """
    try:
...
class VGGT_Long:
    ...
    def process_single_chunk(self, range_1, chunk_idx=None, range_2=None, is_loop=False):
        ...
        save_chunk_result(save_path, predictions)
    ...
    def process_long_sequence(self):
        ...
        for chunk_idx in range(len(self.chunk_indices)-1):
            ...
            chunk_data1 = load_chunk_result(os.path.join(self.result_unaligned_dir, f"chunk_{chunk_idx}.npy"))
            chunk_data2 = load_chunk_result(os.path.join(self.result_unaligned_dir, f"chunk_{chunk_idx+1}.npy"))
        ...
        if self.loop_enable:
            for item in self.loop_predict_list:
                ...
                chunk_data_a = load_chunk_result(os.path.join(self.result_unaligned_dir, f"chunk_{chunk_idx_a}.npy"))
                ...
                chunk_data_b = load_chunk_result(os.path.join(self.result_unaligned_dir, f"chunk_{chunk_idx_b}.npy"))
        ...
        for chunk_idx in range(len(self.chunk_indices) - 1):
            ...
            chunk_data = load_chunk_result(os.path.join(self.result_unaligned_dir, f"chunk_{chunk_idx + 1}.npy"))
            ...
            save_chunk_result(aligned_path, chunk_data)

            if chunk_idx == 0:

                chunk_data_first = load_chunk_result(os.path.join(self.result_unaligned_dir, f"chunk_0.npy"))

                save_chunk_result(os.path.join(self.result_aligned_dir, "chunk_0.npy"), chunk_data_first)
            ...
            aligned_chunk_data = load_chunk_result(os.path.join(self.result_aligned_dir, f"chunk_{chunk_idx+1}.npy")) if chunk_idx > 0 else chunk_data_first
    ...
    def run(self):
        ...
        self.img_list = sorted(glob.glob(os.path.join(self.img_dir, "*.jpg")) +
                               glob.glob(os.path.join(self.img_dir, "*.png")) + glob.glob(os.path.join(self.img_dir, "*.JPG")))
        # print(self.img_list)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_dir', type=str, default='/home/zhanqh/dataset/city/free_dataset/grass/images',
                        help='Image path')
